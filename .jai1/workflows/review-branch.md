---
description: Comprehensive branch/pull request review using specialized agents. Supports both PR review and local branch comparison (e.g., staging → master before deployment)
argument-hint: "[target-branch] [review-aspects]"
---

# Branch / Pull Request Review Instructions

You are an expert code reviewer conducting a thorough evaluation of branch changes or pull request. Your review must be structured, systematic, and provide actionable feedback.

**Arguments (optional):** "$ARGUMENTS"
- First argument can be target branch (e.g., `master`, `main`, `staging`)
- Additional arguments are review aspects (e.g., `security`, `tests`)
...
|------|----------|---------|
| **PR Mode** | Review pull request on GitHub | `/review-branch` (auto-detect PR) |
| **Local Mode** | Compare branches locally (no PR) | `/review-branch master` (compare current → master) |

**Common Local Mode scenarios:**
- Review `staging` → `master/main` before deployment
- Review `feature-branch` → `develop` without creating PR
- Review changes between any two branches locally

## Review Workflow

Run a comprehensive review using multiple specialized agents, each focusing on a different aspect of code quality. Follow these steps precisely:

### Phase 1: Preparation

Run following commands in order:

1. **Determine Review Mode and Scope**
   - Check if running in PR context (GitHub PR exists) or local mode
   - **PR Mode**: Use PR information from GitHub
   - **Local Mode**: Use target branch from arguments (default: `origin/master` or `origin/main`)
   - Check following commands to understand changes:
     - `git status`
     - `git diff --stat`
     - `git diff <target-branch>...HEAD --stat` (e.g., `git diff origin/master...HEAD --stat`)
   - Parse arguments to see if user requested specific review aspects

2. Launch up to 6 parallel Haiku agents to perform following tasks:
   - **PR Mode only**: One agent to check if the pull request (a) is closed, (b) is a draft. If so, do not proceed and return a message that the pull request is not eligible for code review.
   - **Local Mode**: Skip PR eligibility check
   ...
   - Split files based on amount of lines changed between other 1-4 agents and ask them following:
      ```markdown
...
3. **PR Mode only**: If PR missing description, add a description to the PR with summary of changes in short and concise format.
   **Local Mode**: Generate a summary of changes for the review report.

### Phase 2: Searching for Issues

Determine Applicable Reviews, then launch up to 6 parallel (Sonnet or Opus) agents to independently code review all changes in the pull request. Each agent should use the corresponding skill for specialized review instructions.

**Available Review Agents (Skills)**:

| Agent | Skill Reference | Description |
|-------|-----------------|-------------|
| security-auditor | `skill:security-auditor` | Analyze code for security vulnerabilities |
| bug-hunter | `skill:bug-hunter` | Scan for bugs and issues, including silent failures |
| code-quality-reviewer | `skill:code-quality-reviewer` | General code review for project guidelines, maintainability and quality |
| contracts-reviewer | `skill:contracts-reviewer` | Analyze code contracts: type design, invariants, API changes, data modeling |
| test-coverage-reviewer | `skill:test-coverage-reviewer` | Review test coverage quality and completeness |
| historical-context-reviewer | `skill:historical-context-reviewer` | Review historical context via git blame, history, and previous PRs |
...
- **If code or configuration changes, except purely cosmetic changes**: `skill:bug-hunter`, `skill:security-auditor`
- **If code changes, including business or infrastructure logic, formatting, etc.**: `skill:code-quality-reviewer`
- **If test files changed**: `skill:test-coverage-reviewer`
- **If types, API, data modeling changed**: `skill:contracts-reviewer`
- **If complexity of changes is high or historical context is needed**: `skill:historical-context-reviewer`
...
- Each agent loads its corresponding skill for specialized instructions
- Provide to them full list of modified files and summary of the PR as a context, explicitly highlight which PR they are reviewing, also provide list of files with project guidelines and standards, including README.md, CLAUDE.md and constitution.md if they exist.
- Results should come back together

### Phase 3: Confidence & Impact Scoring

1. For each issue found in Phase 2, launch a parallel Haiku agent that takes the branch changes, issue description, and list of CLAUDE.md files (from step 2), and returns TWO scores:
...
3. **PR Mode only**: Use a Haiku agent to repeat the eligibility check from Phase 1, to make sure that the pull request is still eligible for code review. (In case if there was updates since review started)

4. **Output Review Results**:

   **PR Mode** - Post to GitHub:
   ...
   **Local Mode** - Generate local report:
   - Output a comprehensive review report to the console/terminal
   - Use the same format as the PR review template (Quality Gate, scores, issues table)
   - Save report to `reports/branch-review-[timestamp].md` if the `reports/` directory exists
   - Provide actionable next steps (fix issues, then commit/merge)

   When writing output, keep in mind to:
   - Keep your output brief
   - Use emojis
   - Link and cite relevant code, files, and URLs
   - Include the Quality Gate summary, Blocking Issues Count, Security, Test Coverage, and Code Quality scores
